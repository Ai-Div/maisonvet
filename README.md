...
**If you see `/api/auth/error?error=Configuration` ("There is a problem with the server configuration"):** The auth config includes `trustHost: true` for Vercel. Ensure **AUTH_SECRET**, **AUTH_GOOGLE_ID**, and **AUTH_GOOGLE_SECRET** are set in Vercel env vars. If using a custom domain, set **AUTH_URL** to your production URL (e.g. `https://www.maisonvet.com`). Redeploy after adding or changing env vars.
